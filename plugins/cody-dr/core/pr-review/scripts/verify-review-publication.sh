...
#
# The hard part is one distinction. A review comment always belongs to a review,
# so when a reply is posted over REST the platform creates a review to contain
# it and submits it empty. Those shells are attributed to the reviewer and sit
# on the reviewed head, so a naive "exactly one review by this actor" check
# fails a perfectly correct publication. They are containers the platform made,
# not events the reviewer submitted, and they are tolerated up to the number of
# replies the manifest asked for.
#
# What must still fail is a genuine second review. A review is real -- never a
# shell -- if it has a body OR carries inline comments of its own, so an extra
# review with content is reported as `unexpected additional review` even though
# a shell in the same position would have been allowed.
#
...
# A shell is only explicable as the container a REST reply required. More
# shells than replies means something else submitted an empty review.
[[ "$shell_count" -le "$reply_count" ]] \
  || unverified "unexpected additional review: ${shell_count} body-less reviews for ${reply_count} replies"

readonly published="$(jq -c '.[0]' <<<"$real_reviews")"
