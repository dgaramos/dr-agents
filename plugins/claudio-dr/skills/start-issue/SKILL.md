...
**Platform-specific detail (Claude Code only):** use Claude Code branch and
worktree mechanics as documented in the target profile's platform notes, if
any. This instruction is scoped to the Claude Code platform. It is intentionally not mirrored in the Cody DR adapter, which uses Codex shell and file tools for the same step.
