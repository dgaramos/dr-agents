...
case "$*" in
  *addPullRequestReviewThreadReply*)
    printf '%s\n' '{"data":{"addPullRequestReviewThreadReply":{"comment":{"author":{"login":"'"${REPLY_AUTHOR_LOGIN:-cody-dr[bot]}"'"},"pullRequest":{"number":12,"repository":{"nameWithOwner":"octo/example"}}}}}}'
    ;;
  *resolveReviewThread*)
    printf '%s\n' true
    ;;
...
run_action THREAD_ACTION=reply BODY='Unsuffixed login.' REPLY_AUTHOR_LOGIN='cody-dr'
tail -n 1 "$temporary_directory/gh.log" | grep -qF 'addPullRequestReviewThreadReply'

run_action THREAD_ACTION=reply BODY='Suffixed login.' REPLY_AUTHOR_LOGIN='cody-dr[bot]'
tail -n 1 "$temporary_directory/gh.log" | grep -qF 'addPullRequestReviewThreadReply'
